...
def main():
    ...
    for myfile in myfiles:
        ...
        if 'related_dataset' in newdoc:
            ...
            if len(myresults) == 0:
                ...
                logger.warning("Parent found, but assumes parent will be reindexed, thus postponing indexing of children until SolR is updated.")
                myfiles_pending.append(myfile)
                continue
    ...
    if len(myfiles_pending)>0 and not args.always_commit:
        logger.info('Processing files that were not possible to process in first take. Waiting 20 minutes to allow SolR to update recently ingested parent datasets. ')
        sleep(20*60)
